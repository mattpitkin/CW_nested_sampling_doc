...
To calculate the $\Delta\phi$ values, and try to avoid numerical overflow issues when dealing with large
phases, the following equation is used
\begin{equation}
\Delta\phi_j(t) = \sum_{k=1}^n \left( \frac{(f^{(k-1)}_{j,{\rm true}} - f^{(k-1)}_{j,{\rm het}}) }{k!}(t+\delta t_{\rm het})^k + \frac{f^{(k-1)}_{j,{\rm true}}}{k!} \sum_{i=0}^{i<k} \left(\begin{array}k \\ i\end{array} \right) (\Delta t)^{k-i} (t+\delta t_{\rm het})^i \right),
\end{equation}
