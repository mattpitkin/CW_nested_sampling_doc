...
line options for \lppen, as shown with the \verb|--help| command, are given in Table~\ref{tab:commands}.
\begin{footnotesize}
...
\multicolumn{2}{|l|}{~Nested sampling parameters:} \\
 ...
 ~ & ~ \\
\multicolumn{2}{|l|}{~MCMC proposal parameters:} \\
...
\multicolumn{2}{|l|}{~Signal injection parameters:} \\
 ...
 --fake-psd        &  if you want to generate fake data with specific power spectral densities for each detector giving
                     ...
                     then you could use --fake-psd 1e-48,1.5e-48) where values are single-sided PSDs in \verb|Hz^-1| \\
...
The process of performing searches for known pulsars does not just use \lppen. It actually requires a whole pipeline of code to: gather the \gw detector
