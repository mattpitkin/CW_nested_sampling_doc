...
def credible_interval(samples, paramval=None):
...
def get_snr(pfile):
  ...
  snr = 0.
  snrscale = 0. # SNR scale factor from input amplitude (in lppen code)
  fp = open(pfile, 'r')
  ...
  # get scale factor (second to last value) if given on a four value line
  if len(linevals) == 4:
    snrscale = float(linevals[-2])
...
scaleSNRs = {} # scale factors from input SNR to output
...
for j in range(len(dets)):
  ...
  for i in range(nruns):
    ...
    snr, snrscale = get_snr(snrfile)
    snrs.append(snr)
    
    if i == 0 and j < ndets:
      scaleSNRs[dets[j]] = snrscale

  recSNRs.append(np.mean(snrs))
  ...
  try:
    uls.append(ulg(post['H0'].samples))
  except:
  ...
  Zerrs.append(np.sqrt(info/nlive))
  ...
  if injcoh:
    ...
    for vp in varpars:
      ...
      # credible interval
      ci[vp] = credible_interval(post[vp].samples[:,0], ainjpars[dets[0]][vpa])
...
outdic['Injected SNR'] = injsnr
outdic['Recovered SNR'] = recSNRs
outdic['SNR scale factors'] = scaleSNRs # factors by which the amplitude had to be scale to give the injected SNR value
