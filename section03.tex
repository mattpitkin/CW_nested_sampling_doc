...
parameters, and for one chunk of data, is $8.1\ee{-5}$ seconds.\foonote{Without the additional overheads this is a similar time of $4.9\ee{-5}$ seconds.}
...
We will use the median likelihood evaluation time, $\mathcal{T}_{1024}^{L}$ for the test Gaussian likelihood function, as our baseline for timing comparison.
Appropriate scalings can be worked out from the above data.
