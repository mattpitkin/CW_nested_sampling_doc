...
\begin{enumerate}
 ...
 four, whitespace-seperated columns. In either case the first three columns specify the data timestamp in GPS seconds, the real part of
 ...
 required for any variable parameters for which the code needs to sample posterior probability distribtions. An example
...
The prior functions used by the code are decribed in \S\ref{sec:priorfuncs}. To implement them in the code requires a prior
...
\begin{description}
 ...
 normalised to unity, as the code will automatically normalise them). If required this can then be followed by pairs of braketed, comma separated values, for each
...
No whitespace is allowed between detector values or input file values, and they must be seperated by a comma. In this case the output {\tt nest\_H1L1\_SNR} file
