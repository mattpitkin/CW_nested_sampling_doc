...
Here we describe the prior functions allowed for the parameters.

Certain parameters are physically not allowed to take negative values, so the it is hardcoded that the following parameters
...
must either both be positive or both be negative; if using the source model then the two amplitude $I_{21}$ and $I_{31}$, must
have $I_{31} \geqslant I_{21}$.

\subsubsection{Uniform prior}

Figure~\ref{fig:prioruniform}.

\subsubsection{Gaussian prior}

\subsubsection{Gaussian Mixture Model prior}

\subsubsection{Log-Uniform prior}
