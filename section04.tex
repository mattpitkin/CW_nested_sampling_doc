...
In \citet{2014PhRvD..89h4060S} a method was developed for evaluating whether sky location credible intervals for compact binary coalescence signals, produced
...
but given that they are drawn from a flat}, and sees if the credible intervals 
effectively behave as frequentist
...
region bounded by the lower extent to the prior) of the 50\%
credible region. This can test whether the underlying \lalinf parameter sampling is working as expected, or if there are biases to wider or narrow
credible regions. The method, and the ``P-P plots'' it produces by evaluating over a range of credible intervals, has now become commonly used to assess
the \lalinf parameter estimation codes \citep{2015PhRvD..91d2003V} as a way of showing up any problems.

We have used this method here to evaluate our parameters posteriors produced from the simulations performed for \S\label{sec:simsignal}. We have used all
the simulations with coherent SNRs above 10, and, for each, calculated the minimal credible region from the one-dimensional marginalised posterior samples
(using a greedy-binning approach) that the injected value falls within. The cumulative histogram of these values, for each parameter, can be seen in
Figure~\ref{fig:pp_standard}. Also, shown for each parameter is a Kolmogorov-Smirnov test $p$-value comparing our observed distribution to a uniform distribution,
and a ``cloud'' of cumulative distributions that are random realisations of the expected cumulative distribution. We see that, overall, our posteriors seem
to produce posteriors that behave as expected, without any large deviations.
