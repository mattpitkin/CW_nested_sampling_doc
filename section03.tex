...
\begin{align}\label{eq:testgaussev}
 Z &= \int_A^B \frac{1}{\sqrt{2\pi}\sigma}\exp{\left(-\frac{(x-\mu)}{2\sigma^2} \right)}\frac{1}{(B-A)} \text{d}x, \nonumber \\
...
D_{\text{KL}} =& \int_A^B p(x|\mu,\sigma,I) \ln{\left(\frac{p(x|\mu,\sigma,I)}{p(x|I)}\right)} \text{d}x, \nonumber \\ 
 =& \int_A^B \frac{p(\mu,\sigma|x,I)p(x|I)}{Z} \ln{\left(\frac{p(\mu,\sigma|x,I)p(x|I)}{Zp(x|I)}\right)} \text{d}x, \nonumber \\
 =& \frac{p(x|I)}{Z} \int_A^B p(\mu,\sigma|x,I) \left[\ln{\left( p(\mu,\sigma|x,I) \right)} - \ln{Z}\right] \text{d}x, \nonumber \\
...
to
\begin{equation}
D_{\text{KL}} \approx -\frac{1}{4ZB}\left[1+2\left(\ln{Z} +\ln{\left(\sqrt{2\pi\sigma^2}\right)}\right)\right].
\end{equation}
...
increasing number of live points. Linear fits (in $\ln{(Z/Z_{\text{true}})}$ and $D_{\text{KL}}$, the information gain) for different numbers
...
\begin{table}[h]
\caption{Linear coefficients in fits of $\ln{(Z/Z_{\text{true}})} = mD_{\text{KL}} + c$ from the results
