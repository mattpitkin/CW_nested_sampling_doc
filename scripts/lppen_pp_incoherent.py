...
the drawing distribution then p-p plot should still work (problems will occur is the prior is not flat). The sources will be
incoherent between the two detectors - the first detector will contain the "correct" signal, whilst the second signal will
be offset slightly in frequency, have a different h0, phi0, psi and coiota, and have a different sky position to decohere it.
...
rasincoh = 2.*np.pi*np.random.rand(nsigs)
decsincoh = -(np.pi/2.) + np.arccos(2.*np.random.rand(nsigs) - 1.)

# set injection parameters
h0s = np.random.rand(nsigs)*h0fixed
...
# set incoherent parameters
h0sincoh = h0fixed - h0s # h0 between both detectors add to one (or h0fixed anyway!)
phi0sincoh = phi0range[0] + np.diff(phi0range)[0]*np.random.rand(nsigs)  # phi0 value
...
for i in range(nsigs):
  ...
  rastr = pppu.coord_to_string(h, m, s)
  d, m, s = pppu.rad_to_dms(decsincoh[i])
  decstr = pppu.coord_to_string(d, m, s)
